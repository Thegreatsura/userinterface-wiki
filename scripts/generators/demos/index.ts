...
function scanDemos(): DemoEntry[] {
  ...
  for (const article of articles) {
    ...
    for (const demoFolder of demoFolders) {
      ...
      if (!exportName) {
        console.warn(
          `Warning: Could not find export in ${article.name}/demos/${demoFolder.name}`,
        );
        continue;
      }
...
export class DemosGenerator extends Generator {
  ...
  protected async generate(): Promise<GeneratedFile[]> {
    ...
    return [
      {
        name: `demo-registry.ts (${entries.length} demos)`,
        path: OUTPUT_PATH,
        size: stats.size,
      },
    ];
